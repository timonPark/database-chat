...
[단순 조회] curl -sX POST http://localhost:${PORT}/db-query -H 'Content-Type: application/json' -d '{"requestId":"${requestId}","sql":"SELECT col1, col2 FROM table WHERE ... ORDER BY ... FETCH FIRST ${limit} ROWS ONLY","limit":${limit}}'

[집계/조인] curl -sX POST http://localhost:${PORT}/db-aggregate -H 'Content-Type: application/json' -d '{"requestId":"${requestId}","sql":"SELECT t1.col, COUNT(*) FROM t1 JOIN t2 ON t1.id = t2.t1_id WHERE ... GROUP BY ... HAVING ... ORDER BY ... FETCH FIRST ${limit} ROWS ONLY"}'

규칙:
- password·pass_hash·passwd·pwd·secret 컬럼은 반드시 SELECT에서 제외
- SELECT / WITH / EXPLAIN PLAN FOR / SHOW / DESCRIBE / DESC 로 시작하는 읽기 전용 쿼리만 허용 (INSERT·UPDATE·DELETE·DROP·ALTER·CREATE·TRUNCATE·MERGE·GRANT·REVOKE·BEGIN·DECLARE·EXEC·EXECUTE 금지)
- 단순 WHERE·ORDER BY는 /db-query, JOIN·GROUP BY·서브쿼리·HAVING·CTE는 /db-aggregate
- LIMIT 대신 반드시 FETCH FIRST ${limit} ROWS ONLY 사용 (Oracle 12c+ 문법). 더 오래된 버전은 ROWNUM <= ${limit} 사용 가능
- 기본 조회 건수는 반드시 ${limit}건. 엑셀 내보내기는 동일 쿼리를 그대로 실행함
- 날짜 조건은 TO_DATE 함수를 사용한다: TO_DATE('2020-01-01', 'YYYY-MM-DD')
- 성능 최적화 시 EXPLAIN PLAN FOR ... 로 실행 계획을 확인한다
- 숫자로도 문자열로도 저장 가능한 값(전화번호·사번·주민등록번호 등)은 우선 값 그대로 조회한 뒤 결과가 0건이면 타입을 반대로 바꿔 한 번 더 재시도한다. 예) WHERE phone = '01012345678' 로 0건이면 WHERE TO_NUMBER(phone) = 01012345678 또는 WHERE phone = TO_CHAR(01012345678) 로 재조회. 재조회에서도 0건이면 "조회된 데이터가 없습니다"
...
const BLOCKED_SQL_KEYWORDS: RegExp = /\b(INSERT|UPDATE|DELETE|DROP|TRUNCATE|ALTER|CREATE|REPLACE|MERGE|GRANT|REVOKE|BEGIN|DECLARE|EXEC|EXECUTE)\b/i;
...
async function runReadOnlyQuery(sqlStr: string, maxRows?: number): Promise<Record<string, unknown>[]> {
  const conn: oracledb.Connection = await pool.getConnection();
  try {
    const result = await conn.execute(sqlStr, [], {
      outFormat: oracledb.OUT_FORMAT_OBJECT,
      ...(maxRows !== undefined ? { maxRows } : {}),
      fetchArraySize: 100,
    });
    return removeSensitiveColumns((result.rows ?? []) as Record<string, unknown>[]);
  } finally {
...
app.post('/db-query', async (req: Request<object, object, DbQueryBody>, res: Response) => {
  ...
  try {
    ...
    const data: Record<string, unknown>[] = await runReadOnlyQuery(sqlStr, limit);
