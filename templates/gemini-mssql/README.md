...
Antigravity CLI(`agy`) 구독 계정으로 MSSQL(SQL Server)을 조회하는 자연어 채팅 서버 템플릿입니다.
...
| 변수 | 필수 | 기본값 | 설명 |
| --- | --- | --- | --- |
| `DB_HOST` / `DB_PORT` / `DB_DATABASE` / `DB_USER_NAME` / `DB_USER_PASSWORD` | 예 | — | MSSQL 접속 정보 (`DB_PORT`는 보통 1433) |
...
- `LIMIT` 대신 `SELECT TOP n ...` 또는 `... OFFSET 0 ROWS FETCH NEXT n ROWS ONLY` 사용
- 테이블명은 반드시 스키마 포함 (예: `SalesLT.Product`, `dbo.ErrorLog`)
- 날짜 리터럴은 ISO 8601 형식 (`'2020-01-01'`)
