...
- SELECT / WITH / EXPLAIN 으로 시작하는 읽기 전용 쿼리만 허용 (INSERT·UPDATE·DELETE·DROP·ALTER·CREATE·TRUNCATE·COPY·VACUUM·ANALYZE 금지)
...
- 성능 최적화 시 EXPLAIN ANALYZE로 쿼리 계획을 확인하고, 필요하면 부분 인덱스(partial index)를 고려한다
...
function validateSql(sql: string): void {
  ...
  if (!isAllowed) throw new Error('SELECT / WITH / EXPLAIN 으로 시작하는 읽기 전용 쿼리만 허용됩니다.');
...
function isTimeoutError(err: unknown): boolean {
  return (err as { code?: string }).code === '57014'; // PostgreSQL query_canceled
}
...
app.post('/db-query', async (req: Request<object, object, DbQueryBody>, res: Response) => {
  ...
  try {
    ...
    const data: Record<string, unknown>[] = await runReadOnlyQuery(finalSql);
...
app.post('/db-aggregate', async (req: Request<object, object, DbAggregateBody>, res: Response) => {
  ...
  try {
    const dbStart: number = Date.now();
    const data: Record<string, unknown>[] = await runReadOnlyQuery(sql);
    const dbTimeMs: number = Date.now() - dbStart;
