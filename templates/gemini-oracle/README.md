...
Antigravity CLI(`agy`) 구독 계정으로 Oracle Database를 조회하는 자연어 채팅 서버 템플릿입니다.
...
3. **Oracle Instant Client** — `node-oracledb`는 Thick/Thin 모드 중 하나가 필요합니다.
   최신 `oracledb`는 Thin 모드를 기본 지원하므로 별도 클라이언트 없이 접속 가능합니다.
   Wallet/TLS 등 고급 옵션은 [oracledb 문서](https://node-oracledb.readthedocs.io/) 참고.
...
| 변수 | 필수 | 기본값 | 설명 |
| --- | --- | --- | --- |
| `DB_HOST` / `DB_PORT` / `DB_SERVICE_NAME` / `DB_USER_NAME` / `DB_USER_PASSWORD` | 예 | — | Oracle 접속 정보 (`DB_SERVICE_NAME`은 서비스명 또는 SID) |
