#!/usr/bin/env bash
# Claude + Oracle — 시작 스크립트 (macOS/Linux)
# DB Docs: https://docs.oracle.com/en/database/oracle/index.html
cd "$(dirname "$0")"
if [ ! -d node_modules ] || [ ! -f .env ]; then
  echo "초기 설정을 Claude가 자동으로 진행합니다..."
  claude -p "현재 디렉토리에서 순서대로: 1) npm install  2) cp .env.example .env 후 DB_SERVICE_NAME 등 접속 정보 입력  3) docker compose -f docker/docker-compose.yml up -d  4) 완료 시 OK 출력" \
